...
@OnlyIn(Dist.CLIENT)
public class Y_ConquerorHakiFloorParticle extends TexturedCustomModelParticle {
    private LivingEntityPatch<?> caster;

    public Y_ConquerorHakiFloorParticle(ClientLevel level, double x, double y, double z, double xd, double yd, double zd, RawMesh particleMesh, ResourceLocation texture) {
    ...
    public void render(@NotNull VertexConsumer vertexBuffer, Camera camera, float pt) {
        ...
        this.yaw += 0.5F;
        this.scale += (float)Math.max(30 - this.age, 0) * 0.36F;
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(@NotNull SimpleParticleType typeIn, @NotNull ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new Y_ConquerorHakiFloorParticle(level, x, y, z, xSpeed, ySpeed, zSpeed, ESClientModels.CONQUEROR_HAKI_FOOR, new ResourceLocation("guhao", "textures/models/particle/y_conqueror_haki.png"));
        }
    }
}
