...
@OnlyIn(Dist.CLIENT)
public class SpaceConquerorHakiParticle extends TexturedCustomModelParticle {
    private LivingEntityPatch<?> caster;
    public @NotNull ParticleRenderType getRenderType() {
        return EpicACGRenderType.SpaceBrokenEnd;
    }
    public SpaceConquerorHakiParticle(ClientLevel level, double x, double y, double z, double xd, double yd, double zd, RawMesh particleMesh, ResourceLocation texture) {
    ...
    public void render(@NotNull VertexConsumer vertexBuffer, Camera camera, float pt) {
        ...
        this.yaw += 0.5F;
        // Slower scaling - reduced multiplier and age factor
        this.scale += (float)Math.max(10 - this.age, 0) * 0.502F;  // Changed from 1.005F to 0.002F
    ...
    public static class Provider implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(@NotNull SimpleParticleType typeIn, @NotNull ClientLevel level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new SpaceConquerorHakiParticle(level, x, y, z, xSpeed, ySpeed, zSpeed, ESClientModels.CONQUEROR_HAKI, new ResourceLocation("guhao", "textures/models/particle/conqueror_haki.png"));
        }
    }
}
